...
If you'd rather not wait on a CI round-trip, `bash scripts/check.sh` runs everything CI does — the structural lint, the Quickshell-module probe, the headless type-check and both runtime probes — plus a live-session smoke test, in well under a minute. It isn't required to open a PR.
