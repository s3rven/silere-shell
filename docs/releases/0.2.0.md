...
- Accents keep their chroma when blended.
...
- The installer detects a missing terminal properly instead of dying at the first
  prompt.
- The updater's `git fetch` is bounded, so a dead network cannot wedge later checks.
- Updates are type-checked after merging and rolled back if they do not load.
